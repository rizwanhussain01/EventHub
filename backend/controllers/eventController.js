...
// @desc    Get organizer's events (with attendee details)
...
exports.getOrganizerEvents = async (req, res) => {
  try {
    ...
    // For each event, fetch all active tickets + personalDetails
    const eventsWithAttendees = await Promise.all(events.map(async (event) => {
      const attendees = await Ticket.find({ eventId: event._id, status: 'active' })
        .select('personalDetails ticketId userId') // include personal details and ticket info
        .populate('userId', 'name email'); // optionally show user name, email

      return {
        ...event.toObject(),
        attendees // array of tickets with personalDetails
      };
    }));
